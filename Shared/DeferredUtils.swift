...
@discardableResult public func >>== <T, U>(
    x: Deferred<Maybe<T>>, f: @escaping (T) -> Deferred<Maybe<U>>
) -> Deferred<Maybe<U>> {
...
@discardableResult public func >>> <T, U>(
    x: Deferred<Maybe<T>>, f: @escaping () -> Deferred<Maybe<U>>
) -> Deferred<Maybe<U>> {
...
public func walk<T, U, S: Sequence>(
    _ items: S, start: Deferred<Maybe<U>>, f: @escaping (T, U) -> Deferred<Maybe<U>>
...
public func chainResult<T, U>(_ a: Deferred<Maybe<T>>, f: @escaping (T) -> Maybe<U>) -> Deferred<
    Maybe<U>
> {
    return a.map { res in
        if let v = res.successValue {
