...
class NSURLExtensionsTests: XCTestCase {
    ...
    func testoriginalURLFromErrorURL() {
        let goodurls = [
            ...
            (
                "\(InternalURL.baseUrl)/\(InternalURL.Path.errorpage)?url=http%3A//localhost%3A\(AppInfo.webserverPort)/about/home/",
                URL(string: "http://localhost:\(AppInfo.webserverPort)/about/home/")
            ),
        ]
    ...
    func testhavingRemovedAuthorisationComponents() {
        ...
        goodurls.forEach {
            XCTAssertEqual(
                URL(string: $0.0)!.havingRemovedAuthorisationComponents().absoluteString, $0.1)
        }
    }
    ...
    func testnormalizedHostAndPathForDisplay() {
        ...
        goodurls.forEach {
            XCTAssertEqual(URL(string: $0.0)!.normalizedHostAndPathForDisplay, $0.1)
        }
    ...
    func testgetQuery() {
        ...
        params.forEach {
            XCTAssertEqual(
                urlParams[$0], $1, "The values in params should be the same in urlParams")
        }
    }
    ...
    func testHidingFromDataDetectors() {
        ...
        for u in urls {
            ...
            let matches = detector.matches(
                in: original, options: [], range: NSMakeRange(0, original.count))
            ...
            let newMatches = detector.matches(
                in: modified, options: [], range: NSMakeRange(0, modified.count))
