...
class SearchEngineTests: XCTestCase {
    func testSearchURLFrom() throws {
        ...
        // Simplest case
        XCTAssertEqual(searchEngine.searchURLFrom(searchQuery: "abc", queryItems: []),
                       URL(string: "https://\(NeevaConstants.appHost)/search?q=abc&src=nvobar"))

        // Make sure plus signs in the query are encoded
        XCTAssertEqual(searchEngine.searchURLFrom(searchQuery: "c++", queryItems: []),
                       URL(string: "https://\(NeevaConstants.appHost)/search?q=c%2B%2B&src=nvobar"))

        // Make sure query params are not duplicated
        XCTAssertEqual(searchEngine.searchURLFrom(searchQuery: "c++", queryItems: [URLQueryItem(name: "src", value: "nvobar")]),
                       URL(string: "https://\(NeevaConstants.appHost)/search?q=c%2B%2B&src=nvobar"))

        // Make sure additional query params are added
        XCTAssertEqual(searchEngine.searchURLFrom(searchQuery: "c++", queryItems: [URLQueryItem(name: "abc", value: "def")]),
