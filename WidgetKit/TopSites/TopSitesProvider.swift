...
struct TopSitesProvider: TimelineProvider {
    ...
    func getSnapshot(in context: Context, completion: @escaping (TopSitesEntry) -> Void) {
        ...
        let topSitesEntry = TopSitesEntry(
            date: Date(), favicons: tabFaviconDictionary, sites: widgetKitTopSites)
...
extension UIImage {
    fileprivate func withBackgroundAndPadding(color: UIColor, opaque: Bool = true) -> UIImage {
