...
let fingerprintingList =
    "\(rootdir)/../../Carthage/Checkouts/shavar-prod-lists/normalized-lists/base-fingerprinting-track.json"
...
func write(to outputDir: URL, action: Action, categories: [CategoryTitle]) {
    for categoryTitle in categories {
        let result = gen.parseBlocklist(
            json: jsonFrom(filename: blocklist), action: action, categoryTitle: categoryTitle)
        let actionName = action == .blockAll ? "block" : "block-cookies"
        let outputFile =
            "\(outputDir.path)/disconnect-\(actionName)-\(categoryTitle.rawValue.lowercased()).json"
...
write(
    to: outputDir, action: .blockAll,
    categories: [.Advertising, .Analytics, .Social, .Cryptomining])
write(
    to: outputDir, action: .blockCookies, categories: [.Advertising, .Analytics, .Social, .Content])
